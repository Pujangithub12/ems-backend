 ...
 */
export async function migrateSchedulePredecessorsToLinks() {
  console.log("Starting schedule predecessorId -> ScheduleTaskLink migration...");

  const existingLinkCount = await prisma.scheduleTaskLink.count();
  ...
  for (const [projectId, projectTasks] of tasksByProject) {
    ...
    const allProjectTasks = await prisma.scheduleTask.findMany({ where: { projectId } });
    const taskIdToDbId = new Map(allProjectTasks.map((t) => [t.taskId, t.id]));

    const linksToCreate: { projectId: number; predecessorId: number; successorId: number }[] = [];
    ...
    if (linksToCreate.length > 0) {
      await prisma.scheduleTaskLink.createMany({
  ...
  console.log(
    `Migration complete: ${linksCreated} link(s) created across ${projectsTouched} project(s), ` +
