...
const fmtDate = (d?: Date | null) => (d ? d.toLocaleDateString("en-GB", { day: "2-digit", month: "2-digit", year: "numeric" }) : "--");
...
function sectionBar(doc: PDFKit.PDFDocument, x: number, y: number, width: number, height: number, label: string, fontSize = 9) {
...
export function buildPurchaseOrderPdf(po: PurchaseOrderPdfData): PDFKit.PDFDocument {
  ...
  for (const item of po.items) {
    ...
    const rowHeight = Math.max(
      18,
      ...cols.map((col, i) => doc.heightOfString(rowValues[i]!, { width: col.width - 10 }) + 8),
    );
