...
export interface GoodsReceiptPdfItem {
  itemName: string;
  unit?: string | null;
...
const fmtAmount = (n: number) => (n > 0 ? n.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : "--");

// Column widths sum to CONTENT_WIDTH (522).
const COLS = [
  { key: "no", label: "", width: 24, align: "center" as const },
  { key: "goods", label: "Goods", width: 148, align: "left" as const },
  { key: "pack", label: "Pack Size", width: 55, align: "center" as const },
  { key: "price", label: "Price", width: 55, align: "right" as const },
  { key: "order", label: "Order Quantity", width: 60, align: "right" as const },
  { key: "delivered", label: "Delivered Quantity", width: 68, align: "right" as const },
  { key: "comments", label: "Comments", width: 112, align: "left" as const },
];
 ...
 */
export function buildGoodsReceiptPdf(gr: GoodsReceiptPdfData): PDFKit.PDFDocument {
  const doc = new PDFDocument({ size: [PAGE_WIDTH, PAGE_HEIGHT], margins: { top: MARGIN, right: MARGIN, bottom: MARGIN, left: MARGIN } });
  let y = MARGIN;
  ...
  for (const col of COLS) {
    ...
    if (col.label) doc.text(col.label, colX + 4, y + (headerH - 10) / 2, { width: col.width - 8, align: col.align });
    colX += col.width;
  }
  y += headerH;
  ...
  gr.items.forEach((item, i) => {
    ...
    for (const col of COLS) {
      ...
      if (value) doc.text(value, colX + 4, y + 6, { width: col.width - 8, align: col.align });
      colX += col.width;
    }
    y += rowH;
  });
