...
exports.backfillTaskLinksForMigratedProjects = backfillTaskLinksForMigratedProjects;
 ...
 */
async function backfillTaskLinksForMigratedProjects() {
    console.log("Starting TaskLink backfill for already-migrated projects...");
    const migratedTasks = await prisma_1.prisma.task.findMany({
        where: { migratedFromScheduleTaskId: { not: null } },
        select: { id: true, projectId: true, migratedFromScheduleTaskId: true },
    });
    const projectIds = Array.from(new Set(migratedTasks.map((t) => t.projectId).filter((id) => id != null)));
    let projectsTouched = 0;
    let projectsSkipped = 0;
    let linksCreated = 0;
    for (const projectId of projectIds) {
        const existingLinkCount = await prisma_1.prisma.taskLink.count({ where: { projectId } });
        if (existingLinkCount > 0) {
            projectsSkipped += 1;
            continue;
        }
        const legacyLinks = await prisma_1.prisma.scheduleTaskLink.findMany({ where: { projectId } });
        if (legacyLinks.length === 0)
            continue;
        const legacyIdToNewTaskId = new Map(migratedTasks
            .filter((t) => t.projectId === projectId)
            .map((t) => [t.migratedFromScheduleTaskId, t.id]));
        ...
        if (linksToCreate.length > 0) {
            await prisma_1.prisma.taskLink.createMany({ data: linksToCreate });
            linksCreated += linksToCreate.length;
            projectsTouched += 1;
        }
    }
    console.log(`Backfill complete: ${linksCreated} link(s) created across ${projectsTouched} project(s), ${projectsSkipped} project(s) skipped (already had links).`);
}
...
//# sourceMappingURL=backfill-tasklinks-for-migrated-projects.js.map