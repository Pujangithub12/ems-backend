...
exports.buildProformaInvoicePdf = buildProformaInvoicePdf;
exports.proformaInvoicePdfBuffer = proformaInvoicePdfBuffer;
 ...
 */
function buildProformaInvoicePdf(pi) {
    const doc = new pdfkit_1.default({ size: [PAGE_WIDTH, PAGE_HEIGHT], margins: { top: MARGIN_TOP, right: MARGIN_RIGHT, bottom: MARGIN_BOTTOM, left: MARGIN_LEFT } });
    ...
    const cols = [
        ...
        { label: "Qty", width: colW(1000), align: "right" },
        { label: "Unit", width: colW(900) },
        { label: "Unit Price (" + (pi.currency?.trim() || "NPR") + ")", width: colW(1600), align: "right" },
        { label: "Amount (" + (pi.currency?.trim() || "NPR") + ")", width: colW(1654), align: "right" },
    ];
    let colX = MARGIN_LEFT;
    doc.font(FONT_COMPANY).fontSize(8.5);
    const headerH = Math.max(22, ...cols.map((col) => doc.heightOfString(col.label, { width: col.width - 8, align: col.align || "left" }) + 12));
    ensureSpace(headerH);
    ...
    for (const item of pi.items) {
        ...
        const rowHeight = Math.max(16, ...cols.map((col, i) => doc.font(FONT_BODY).fontSize(8.5).heightOfString(rowValues[i], { width: col.width - 8 }) + 8));
    ...
    y += totalRowH + 8;
    // ---- Amount in Words ----
    const amountWordsText = `Amount in words: ${(0, numberToWords_1.amountToWords)(grandTotal, pi.currency?.trim() || "Rupees")}.`;
    ...
    const boxTop = y + 16;
    const renderBoxLines = (x, w, lines) => {
        let ly = boxTop + 8;
        for (const [label, value] of lines) {
...
function proformaInvoicePdfBuffer(pi) {
...
//# sourceMappingURL=proformaInvoicePdf.js.map