...
exports.migrateScheduleToTask = migrateScheduleToTask;
 ...
 */
async function migrateScheduleToTask() {
    console.log("Starting schedule_task -> task migration...");
    const legacyRows = await prisma_1.prisma.scheduleTask.findMany({
    ...
    for (const [projectId, rows] of rowsByProject) {
        const alreadyMigrated = await prisma_1.prisma.task.findFirst({
        ...
        await prisma_1.prisma.$transaction(async (tx) => {
            ...
            for (const legacy of rows) {
                ...
                if (!parentLegacy)
                    continue; // dangling reference in legacy data — leave unparented
...
//# sourceMappingURL=migrate-schedule-to-task.js.map