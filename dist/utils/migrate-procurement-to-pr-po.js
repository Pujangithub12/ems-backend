...
exports.migrateProcurementToPrPo = migrateProcurementToPrPo;
 ...
 */
async function migrateProcurementToPrPo() {
    console.log("Starting procurement -> purchase request/order migration...");
    const legacyItems = await prisma_1.prisma.procurementItem.findMany({
    ...
    for (const legacy of legacyItems) {
        const alreadyMigrated = await prisma_1.prisma.purchaseRequest.findUnique({
        ...
        const prStatus = legacy.status === "delivered" || legacy.status === "ordered" ? "converted_to_po" : legacy.status === "approved" ? "approved" : "submitted";
    ...
    console.log(`Migration complete: ${migratedPrOnly} PR-only, ${migratedWithPo} with PO, ${migratedWithGrn} with GRN, ${skipped} skipped (already migrated or orphaned).`);
}
...
//# sourceMappingURL=migrate-procurement-to-pr-po.js.map