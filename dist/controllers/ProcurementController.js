...
const procurementCost_1 = require("../utils/procurementCost");
...
const decimalToNumber = (value) => value == null ? null : value.toNumber();
...
class ProcurementController {
    ...
    static getOrganizationProcurement = async (req, res) => {
        try {
            const items = await prisma_1.prisma.procurementItem.findMany({
    ...
    static getProcurementItems = async (req, res) => {
    ...
    static addProcurementItem = async (req, res) => {
        const { projectId } = req.params;
        const { itemName, itemId, category, quantity, unit, estimatedCost, unitCost, taxPercent, discountPercent, transportCost, customsCost, vendorName, vendorId, neededByDate, notes, } = req.body;
        try {
            const project = await prisma_1.prisma.project.findFirst({
                where: {
                    id: parseInt(projectId),
            ...
            const vendor = vendorId
                ...
                : null;
            const createdRow = await prisma_1.prisma.procurementItem.create({
                data: {
            ...
            const updatedRow = await prisma_1.prisma.procurementItem.update({
                where: { id: createdRow.id },
                data: { poNumber },
            });
            await prisma_1.prisma.procurementStatusHistory.create({
                data: {
    ...
    static updateProcurementItem = async (req, res) => {
        const { itemId } = req.params;
        const { itemName, itemId: catalogItemId, category, quantity, unit, estimatedCost, unitCost, taxPercent, discountPercent, transportCost, customsCost, vendorName, vendorId, neededByDate, status, notes, } = req.body;
        try {
            const existing = await prisma_1.prisma.procurementItem.findFirst({
            ...
            if (quantity !== undefined)
                data.quantity = Math.round(quantity) || 1;
            if (unit !== undefined)
                data.unit = unit;
            ...
            if (vendorId !== undefined) {
                if (vendorId === null) {
                    ...
                    const foundVendor = await prisma_1.prisma.vendor.findFirst({
                        where: { id: vendorId, organizationId: req.organization.id },
                    });
                    if (foundVendor) {
            ...
            if (catalogItemId !== undefined) {
                if (catalogItemId === null) {
                    ...
                    const foundCatalogItem = await prisma_1.prisma.catalogItem.findFirst({
            ...
            if (status !== undefined && status !== previousStatus) {
                ...
                if (status === "delivered") {
                    await InventoryController_1.InventoryController.receiveFromProcurement({
                        ...
                        unitCost: (0, procurementCost_1.computeLandedUnitCost)({
    ...
    static deleteProcurementItem = async (req, res) => {
        const { itemId } = req.params;
        try {
            const item = await prisma_1.prisma.procurementItem.findFirst({
                where: { id: parseInt(itemId) },
            ...
            await prisma_1.prisma.procurementItem.delete({ where: { id: item.id } });
    ...
    static addAttachment = async (req, res) => {
        ...
        try {
            const item = await ProcurementController.loadOwnedItem(itemId, req.organization.id);
            if (!item)
                return res.status(404).json({ message: "Procurement item not found" });
            const uploadedBy = await prisma_1.prisma.user.findUnique({ where: { id: req.user.id } });
            const attachment = await prisma_1.prisma.procurementAttachment.create({
                data: {
    ...
    static deleteAttachment = async (req, res) => {
        ...
        try {
            ...
            if (!item)
                return res.status(404).json({ message: "Procurement item not found" });
            const attachment = await prisma_1.prisma.procurementAttachment.findFirst({
                where: { id: parseInt(attachmentId), procurementItemId: item.id },
            });
            if (!attachment)
                return res.status(404).json({ message: "Attachment not found" });
            await prisma_1.prisma.procurementAttachment.delete({ where: { id: attachment.id } });
     ...
     */
    static getProcurementItemDetail = async (req, res) => {
        const { itemId } = req.params;
        try {
            const item = await ProcurementController.loadOwnedItem(itemId, req.organization.id);
            if (!item)
            ...
            const [statusHistory, attachments, projectAllocation] = await Promise.all([
                prisma_1.prisma.procurementStatusHistory.findMany({
                    where: { procurementItemId: item.id },
                    include: { changedBy: true },
                    orderBy: { createdAt: "desc" },
                }),
                prisma_1.prisma.procurementAttachment.findMany({
                    where: { procurementItemId: item.id },
