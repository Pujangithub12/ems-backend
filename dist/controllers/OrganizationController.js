...
class OrganizationController {
    ...
    static async update(req, res) {
        try {
            ...
            const isMember = await prisma_1.prisma.organizationMembership.findFirst({
            ...
            const updatedOrganization = await prisma_1.prisma.organization.update({
    ...
    static async remove(req, res) {
        try {
            ...
            await prisma_1.prisma.organization.delete({ where: { id: organizationId } });
    ...
    static async getAccessMatrix(req, res) {
        try {
            ...
            const allMemberships = organizationIds.length > 0
                ? await prisma_1.prisma.organizationMembership.findMany({
    ...
    static async grantMemberAccess(req, res) {
        try {
            ...
            const role = (roleInput || enums_1.UserRole.USER);
            ...
            const actorMembership = await prisma_1.prisma.organizationMembership.findFirst({
                where: { organizationId, userId: req.user.id },
            });
            if (!actorMembership) {
                return res.status(403).json({ message: "Not a member of this organization" });
            ...
            const existingMembership = await prisma_1.prisma.organizationMembership.findFirst({
            ...
            if (role === enums_1.UserRole.SUPER_ADMIN) {
                const existingSuperAdmins = await (0, UserController_1.countSuperAdminsInOrganization)(organizationId);
    ...
    static async revokeMemberAccess(req, res) {
        try {
            const organizationId = Number(req.params.id);
            const userId = Number(req.params.userId);
            if (!Number.isInteger(organizationId) || !Number.isInteger(userId)) {
            ...
            const targetMembership = await prisma_1.prisma.organizationMembership.findFirst({
                where: { organizationId, userId },
            });
            if (!targetMembership) {
                return res.status(400).json({ message: "This user doesn't have access to this organization" });
            }
            const targetMembershipCount = await prisma_1.prisma.organizationMembership.count({
                where: { userId },
            });
            if (targetMembershipCount <= 1) {
                return res.status(400).json({
                    message: "This is their only organization — remove them from Users instead if you want to revoke all access.",
                });
            }
            await prisma_1.prisma.organizationMembership.delete({ where: { id: targetMembership.id } });
