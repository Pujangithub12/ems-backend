...
class ShipmentController {
    ...
    static addShipment = async (req, res) => {
        const { id } = req.params;
        const { transportMode, transportCompany, containerNo, vehicleNo, trackingNo, etd, eta, arrivalDate, status, freightCost, loadingCost, unloadingCost, fuelCost, miscellaneousCost, localTaxCost, } = req.body;
        try {
            ...
            const created = await prisma_1.prisma.shipment.create({
                data: {
    ...
    static updateShipment = async (req, res) => {
        ...
        const { transportMode, transportCompany, containerNo, vehicleNo, trackingNo, etd, eta, arrivalDate, status, freightCost, loadingCost, unloadingCost, fuelCost, miscellaneousCost, localTaxCost, } = req.body;
        try {
            const existing = await ShipmentController.loadOwnedShipment(id, req.organization.id);
            if (!existing)
                return res.status(404).json({ message: "Shipment not found" });
            const data = {};
            if (transportMode !== undefined)
                data.transportMode = transportMode;
            if (transportCompany !== undefined)
            ...
            if (arrivalDate !== undefined)
                data.arrivalDate = arrivalDate ? new Date(arrivalDate) : null;
            if (status !== undefined)
                data.status = status;
            if (freightCost !== undefined)
                data.freightCost = freightCost;
            if (loadingCost !== undefined)
                data.loadingCost = loadingCost;
            if (unloadingCost !== undefined)
                data.unloadingCost = unloadingCost;
            if (fuelCost !== undefined)
                data.fuelCost = fuelCost;
            if (miscellaneousCost !== undefined)
                data.miscellaneousCost = miscellaneousCost;
            if (localTaxCost !== undefined)
                data.localTaxCost = localTaxCost;
            const shipment = await prisma_1.prisma.shipment.update({
    ...
    static addInsurance = async (req, res) => {
        const { id } = req.params;
        const { insuranceCompany, policyNumber, coverage, premium, claimStatus } = req.body;
        try {
            ...
            const insurance = await prisma_1.prisma.insurance.create({
                data: {
    ...
    static async loadOwnedInsurance(id, organizationId) {
        const insurance = await prisma_1.prisma.insurance.findFirst({
            where: { id: parseInt(id) },
            include: { shipment: { include: { purchaseOrder: true } } },
        });
        if (!insurance || insurance.shipment?.purchaseOrder?.organizationId !== organizationId)
            return null;
        return insurance;
    }
    ...
    static updateInsurance = async (req, res) => {
        ...
        const { insuranceCompany, policyNumber, coverage, premium, claimStatus } = req.body;
        try {
            const existing = await ShipmentController.loadOwnedInsurance(id, req.organization.id);
            if (!existing)
            ...
            const insurance = await prisma_1.prisma.insurance.update({
                where: { id: existing.id },
                data,
            });
    ...
    static addCustoms = async (req, res) => {
        const { id } = req.params;
        const { customDeclarationNumber, billOfEntry, hsCode, clearingAgent, port, importDuty, vat, excise, serviceCharge, documentationCost, inspectionCost, warehouseCost, miscellaneousCost, } = req.body;
        try {
            const shipment = await ShipmentController.loadOwnedShipment(id, req.organization.id);
            if (!shipment)
                return res.status(404).json({ message: "Shipment not found" });
            ...
            const customs = await prisma_1.prisma.customs.create({
                data: {
                    shipmentId: shipment.id,
    ...
    static async loadOwnedCustoms(id, organizationId) {
        const customs = await prisma_1.prisma.customs.findFirst({
            where: { id: parseInt(id) },
            include: { shipment: { include: { purchaseOrder: true } } },
        });
        if (!customs || customs.shipment?.purchaseOrder?.organizationId !== organizationId)
            return null;
        return customs;
    }
    /** PUT /customs/:id — admin-gated. */
    static updateCustoms = async (req, res) => {
        const { id } = req.params;
        const { customDeclarationNumber, billOfEntry, hsCode, clearingAgent, port, importDuty, vat, excise, serviceCharge, documentationCost, inspectionCost, warehouseCost, miscellaneousCost, } = req.body;
        try {
            const existing = await ShipmentController.loadOwnedCustoms(id, req.organization.id);
            if (!existing)
                return res.status(404).json({ message: "Customs not found" });
            const data = {};
            if (customDeclarationNumber !== undefined)
                data.customDeclarationNumber = customDeclarationNumber;
            if (billOfEntry !== undefined)
                data.billOfEntry = billOfEntry;
            if (hsCode !== undefined)
                data.hsCode = hsCode;
            if (clearingAgent !== undefined)
                data.clearingAgent = clearingAgent;
            if (port !== undefined)
                data.port = port;
            if (importDuty !== undefined)
                data.importDuty = importDuty;
            if (vat !== undefined)
                data.vat = vat;
            if (excise !== undefined)
                data.excise = excise;
            ...
            if (documentationCost !== undefined)
                data.documentationCost = documentationCost;
            if (inspectionCost !== undefined)
                data.inspectionCost = inspectionCost;
            if (warehouseCost !== undefined)
                data.warehouseCost = warehouseCost;
            if (miscellaneousCost !== undefined)
                data.miscellaneousCost = miscellaneousCost;
            const customs = await prisma_1.prisma.customs.update({
                where: { id: existing.id },
                data,
            });
    ...
    static addCustomsDocument = async (req, res) => {
        ...
        try {
            const existing = await ShipmentController.loadOwnedCustoms(itemId, req.organization.id);
            if (!existing)
                return res.status(404).json({ message: "Customs not found" });
            const document = await prisma_1.prisma.customsDocument.create({
                data: {
    ...
    static deleteCustomsDocument = async (req, res) => {
        const { itemId, documentId } = req.params;
        try {
            const existing = await ShipmentController.loadOwnedCustoms(itemId, req.organization.id);
            if (!existing)
                return res.status(404).json({ message: "Customs not found" });
            const document = await prisma_1.prisma.customsDocument.findFirst({
                where: { id: parseInt(documentId), customsId: existing.id },
            });
            if (!document)
                return res.status(404).json({ message: "Document not found" });
            await prisma_1.prisma.customsDocument.delete({ where: { id: document.id } });
